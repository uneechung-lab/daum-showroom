...
// 2. Fix the GSAP animation script exactly using string replace
const gsapTarget = `.to("#hero-showcase-box", { 
                  width: "360px", 
                  height: "821px", 
                  x: 170,
                  borderRadius: "32px",`;
...
if (axLab.includes(gsapTarget)) {
    axLab = axLab.replace(gsapTarget, newGsap);
} else {
    console.log("Could not find GSAP target");
}

const iconGsapTarget = `.fromTo(".icon-particle", { opacity: 0, scale: 0.5 }, { opacity: 1, scale: 1, x: (i) => [-300, 300, 0][i], y: (i) => [-140, -140, 110][i],`;
...
if (axLab.includes(iconGsapTarget)) {
    axLab = axLab.replace(iconGsapTarget, newIconGsap);
} else {
    console.log("Could not find Icon GSAP target");
}
