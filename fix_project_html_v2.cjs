...
const markerStart = '      .content-wrapper {';
const markerEnd = '      <div class="container">';

const idxStart = content.indexOf(markerStart);
const idxEnd = content.indexOf(markerEnd, idxStart);
...
if (idxStart !== -1 && idxEnd !== -1) {
  ...
  <body class="sub-page force-dark">
    ...
    content = content.substring(0, idxStart) + replacement + content.substring(idxEnd + markerEnd.length);
    fs.writeFileSync('c:\\daum_site\\project.html', content);
    console.log('Successfully fixed project.html parsing error!');
} else {
